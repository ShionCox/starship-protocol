...
```text
BootScene ──进入──> MainScene ──战斗──> BattleScene
                      │                    │
                 ShipView.prefab      两个 ShipView.prefab
                      └────── 共享 UIRoot.prefab ──────┘
```

- Scene 只定义运行模式；船体由 `HullDefinition + 船体外观 Prefab + ShipSnapshot` 定义，不为每种船体复制 Scene。
- `ShipModel` 是单舰聚合根；实例短 ID 只需在所属 `shipId` 内唯一。
- Main/Battle 的 UI 通过 `{ shipId, readState, commandHandler }` 显式绑定，不搜索全场景 RoomView/CrewView。
- UI 与 Cocos View 只依赖 `PlayerStatePort`。本地存储和未来 HTTP 都是端口适配器，不能反向进入 GameCore。
- Bootstrap 只连接 Creator 已保存的组件和引用；缺失即中文报错并停止，禁止运行时拼装正式场景。

完整决策见 `ADR-0004-R1客户端重基线与服务器边界.md`。
...
```ts
type GameCommand =
  | { type: 'MOVE_CREW'; shipId: string; crewInstanceId: string; targetRoomInstanceId: string }
  | { type: 'SET_ROOM_POWER'; shipId: string; roomInstanceId: string; power: number }
  | { type: 'SET_WEAPON_TARGET'; sourceShipId: string; roomInstanceId: string; targetShipId: string; targetRoomInstanceId: string }
  | { type: 'USE_ABILITY'; shipId: string; crewInstanceId: string; abilityId: string };
```
...
```ts
type GameEvent =
  | { type: 'CREW_MOVE_STARTED'; shipId: string; crewInstanceId: string; from: string; to: string }
  | { type: 'WEAPON_FIRED'; sourceShipId: string; weaponId: string; targetShipId: string; targetRoomInstanceId: string }
  | { type: 'PROJECTILE_HIT'; targetShipId: string; projectileId: string; targetRoomInstanceId: string }
  | { type: 'ROOM_DAMAGED'; shipId: string; roomInstanceId: string; value: number }
  | { type: 'CREW_DAMAGED'; shipId: string; crewInstanceId: string; value: number }
