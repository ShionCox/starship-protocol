...
- `extensions/starship-editor-tools/`：一个插件宿主、一个房间领域模块；资源菜单打开原生中文 Panel，使用公开 Asset DB 消息创建 JSON 与复制 Prefab。
...
覆盖菜单聚合和目录传递、非法路径、非法名称、重名拒绝、JSON + Prefab 成功创建、Prefab 复制失败回滚、回滚失败可观察，以及当前 Prefab 的 RoomView 定义绑定校验。
...
- [x] 历史截图记录扩展管理器识别并启用 `starship-editor-tools v1.1.0`；当前源包版本为 v1.2.0。证据：[`M0-007-extension-manager-enabled.jpg`](M0-007-extension-manager-enabled.jpg)；禁用回归完成后已恢复启用，见 [`M0-007-extension-manager-reenabled.jpg`](M0-007-extension-manager-reenabled.jpg)。
...
- [x] 禁用插件后，Web Desktop 仍可构建和运行。证据：[`M0-007-plugin-disabled-build-verification.md`](M0-007-plugin-disabled-build-verification.md)、[`M0-007-extension-manager-disabled.jpg`](M0-007-extension-manager-disabled.jpg)、[`M0-007-plugin-disabled-web-runtime.png`](M0-007-plugin-disabled-web-runtime.png)。
