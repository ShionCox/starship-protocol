...
test('创作面板只在可见期间轮询选择，选择未变化时不重复请求', async () => {
  ...
  const panel = {
    $: {
      ...
      sceneKind: element(), initialize: element(), createFoundation: element(), mountSharedUi: element(), wireFoundation: element(), sceneRefresh: element(), refresh: element(), list: element(), status: element(),
  ...
  assert.equal(removedBroadcastListeners.length, 3);
