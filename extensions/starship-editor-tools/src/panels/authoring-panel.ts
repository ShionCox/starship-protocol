  ...
  ready() {
    ...
    const renderInspector = (): void => {
      ...
      if (entry === undefined) {
        ...
        roomInstanceInfo.textContent = `实例 ID：${roomSelection?.instanceId ?? '未读取'}\n逻辑格：${formatGridPosition(roomSelection?.gridPosition)}\n校验：${roomSelection?.validation.message ?? '未读取'}\n${roomSelection?.definitionFound === false ? '错误：共享房间定义未进入资源目录，实例字段仍保留只读显示。' : ''}`;
        for (const key of ['editDisplayName', 'editCategory', 'editWidth', 'editHeight', 'editMaxLevel', 'editMaxHp', 'editMinPower', 'editMaxPower', 'editCrewCapacity']) field(key).value = '';
        return;
      }
      ...
      roomInstanceInfo.textContent = roomSelection === undefined ? '' : `实例 ID：${roomSelection.instanceId ?? '未读取'}\n节点路径：${roomSelection.path ?? '—'}\n逻辑格：${formatGridPosition(roomSelection.gridPosition)}\n校验：${roomSelection.validation.message}`;
    ...
    const renderSceneInspector = (selectionValue: AuthoringSelection): void => {
      sceneInspector.hidden = selectionValue.kind === 'room-instance';
    ...
    const render = (next: AuthoringState): void => {
      ...
      state = next; roomEntries = next.rooms;
...
function translateSemanticRole(role: string): string {
  return ({ mainCamera: '主相机', canvas: '画布', background: '背景层', worldRoot: '世界根', shipRoot: '飞船根', gridRoot: '网格根', roomRoot: '房间容器', previewRoot: '预览根', uiRoot: '界面根', appRoot: '应用根' } as Record<string, string>)[role] ?? role;
}
